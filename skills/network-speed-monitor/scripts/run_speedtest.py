...
def run_measurement() -> dict:
    # Try primary CLI first (speedtest-cli / speedtest)
    cmd = find_cli(prefer_librespeed=False)
    cli_used = Path(cmd[0]).name
    cli_name = Path(cmd[0]).name.lower()
    
    # Check if we're already using librespeed (fallback already in use)
    using_librespeed = "librespeed" in cli_name
    
    # Add server preference for Ookla/speedtest-cli (not librespeed)
    server_arg = None
    if not using_librespeed:
        server_arg = find_preferred_server(cmd)
    
    if cmd and cmd[0].endswith("speedtest-cli"):
        cmd.insert(1, "--secure")
    
    # Insert server arg if found (before --json/--format)
    if server_arg:
        # Find position to insert (before --json or --format=json)
        insert_pos = len(cmd)
    ...
    base_attempts = [cmd[:], cmd[:]]
    if base_attempts[1] and base_attempts[1][0].endswith("speedtest-cli") and "--secure" in base_attempts[1]:
        base_attempts[1] = [part for part in base_attempts[1] if part != "--secure"]

    last_error = None
    for round_idx in range(3):
        for attempt_cmd in base_attempts:
            proc = subprocess.run(attempt_cmd, capture_output=True, text=True, timeout=420)
            stdout = (proc.stdout or "").strip()
            stderr = (proc.stderr or "").strip()
            if proc.returncode != 0:
                last_error = stderr or stdout or f"speedtest command failed with code {proc.returncode}"
                continue
            if not stdout:
                last_error = f"speedtest returned empty output for command: {' '.join(attempt_cmd)}"
                continue
            try:
                data = json.loads(stdout)
                break
            except json.JSONDecodeError:
                last_error = f"non-JSON output from speedtest. stdout={stdout[:300]!r} stderr={stderr[:300]!r}"
        else:
            # Check if this is a 403/speedtest.net blocking error and we haven't tried librespeed yet
            if last_error and ('403' in last_error or 'configuration' in last_error.lower() or 'forbidden' in last_error.lower()):
                if not using_librespeed:
                    # Try falling back to librespeed-cli
                    try:
                        return run_librespeed_fallback()
                    except Exception as librespeed_err:
                        last_error = f"speedtest.net failed (403), librespeed fallback also failed: {librespeed_err}"
                time.sleep(5 * (round_idx + 1))
                continue
            raise RuntimeError(last_error or "speedtest command failed")
        break
    else:
        # If all speedtest.net attempts failed and we haven't tried librespeed, try it now
        if not using_librespeed and last_error and ('403' in last_error or 'configuration' in last_error.lower() or 'forbidden' in last_error.lower()):
            try:
                return run_librespeed_fallback()
            except Exception as librespeed_err:
                raise RuntimeError(f"{last_error or 'speedtest command failed'} after 3 tries; librespeed fallback: {librespeed_err}")
        raise RuntimeError(f"{last_error or 'speedtest command failed'} after 3 tries")

    # Normalize Ookla, speedtest-cli, and LibreSpeed formats.
    # LibreSpeed returns Mbps directly; Ookla/speedtest-cli return bps.
    ...
    # Convert bps to Mbps unless we already have Mbps (LibreSpeed)
...
def append_logs(row: dict) -> None:
    ...
    with CSV_PATH.open("a", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["timestamp", "download_mbps", "upload_mbps", "ping_ms", "status", "target_delta_mbps", "server", "isp", "external_ip", "cli_used", "server_host", "server_country", "summary"],
        )
        if write_header:
            writer.writeheader()
        writer.writerow(row)
...
def build_summary(current: dict, history: list[dict]) -> str:
    recent = [h for h in history if h.get("download_mbps") is not None]
